...
def playlist():
    try:
        ...
        if r.status_code != 200:
            return f"❌ Token Expired ({r.status_code})", 403
            
        # VERCEL FIX: Instead of proxying .ts files through the server (which times out),
        # we provide the absolute URL. 
        # Note: This only works if your browser can bypass CORS or if the CDN allows it.
        base_folder = PLAYLIST_URL.rsplit('/', 1)[0]
        content = r.text.replace('master_ap_', f'{base_folder}/master_ap_')
        return Response(content, mimetype='application/vnd.apple.mpegurl')
    except Exception as e:
        return str(e), 500

# Vercel needs this to recognize the app
app.debug = False
